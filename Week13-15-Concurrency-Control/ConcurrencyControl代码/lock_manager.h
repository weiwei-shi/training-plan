...
class LockManager {
  ...
  class LockRequestQueue {
   public:
    ...
    bool hasSharedLock(){
      std::list<LockRequest>::iterator curr = request_queue_.begin();
      for(;curr!=request_queue_.end();curr++){
        if(curr->lock_mode_ == LockMode::SHARED && curr->granted_){
          return true;
        }
      }
      return false;
 ...
 private:
  ...
  std::unordered_set<txn_id_t> cycle_point_set;
  std::unordered_set<txn_id_t> visited;
  txn_id_t id;
  std::unordered_map<txn_id_t, RID> txn_tuple;
};
