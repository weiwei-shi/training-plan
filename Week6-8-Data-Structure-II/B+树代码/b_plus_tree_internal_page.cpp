...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(const ValueType &old_value, const KeyType &new_key,
  ...
  for(int i = current_size - 1;i > (old_index + 1);i--){
    array[i] = array[i-1];
  }
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(BPlusTreeInternalPage *recipient,
  ...
  recipient->CopyNFrom(&array[start], moved_num, buffer_pool_manager);
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyNFrom(MappingType *items, int size, BufferPoolManager *buffer_pool_manager) {
  for(int i = 0;i < size;i++){
    ...
    Page *childPage = buffer_pool_manager->FetchPage(array[GetSize() + i].second);
    BPlusTreePage *childTreePage = reinterpret_cast<BPlusTreePage *>(childPage->GetData());
    childTreePage->SetParentPageId(GetPageId());
    buffer_pool_manager->UnpinPage(array[GetSize() + i].second,true);
  }
  IncreaseSize(size);
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyLastFrom(const MappingType &pair, BufferPoolManager *buffer_pool_manager) {
  ...
  BPlusTreePage *childTreePage = reinterpret_cast<BPlusTreePage *>(childPage);
  childTreePage->SetParentPageId(GetPageId());
